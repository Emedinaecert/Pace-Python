...
def establish(connection, password):
   ...
   # )
    mse_set_at_capdu = bytes(
        [0x00, 0x22, 0xc1, 0xa4, 0x12, 0x80, len(ALGORITHM_OID)]
        + ALGORITHM_OID
        + [0x83, 0x01, PASSWORD_REFERENCE]
        + [0x84, 0x01, 0x0f]
    )
    ...
    standard_base_point = NIST384p.generator.to_affine()
    ...
    perform_key_agreement_dad = build_dynamic_authentication_data(bytes([0x83]), uncompressed_public_key)    #GA3
    ...
    shared_secret = calculated_secret_point.x().to_bytes(48, byteorder='big')
    ...
    terminal_authentication_token_MAC = calculate_authentication_token(Kmac, chip_uncompressed_point) # DIGEST
...
def map_nonce(connection, decrypted_nonce, curve, standard_base_point, order):
    ...
    map_nonce_rapdu = util.send_command(connection, map_nonce_capdu)# GA2
    chip_uncompressed_point = map_nonce_rapdu[4:]

    chip_public_key = decode_uncompressed_point(chip_uncompressed_point, curve, order)  #KDF GA2
    shared_secret = chip_public_key * int.from_bytes(private_key, 'big')

    mapped_base_point = standard_base_point * int.from_bytes(decrypted_nonce, 'big') + shared_secret  #G''  -> n*G + H
    return mapped_base_point


def generate_key_pair(base_point):
    private_key = os.urandom(48)
...
def encode_uncompressed_point(point):
    return bytes([0x04]) + point.x().to_bytes(48, byteorder='big') + point.y().to_bytes(48, byteorder='big')


def decode_uncompressed_point(bytes, curve, order):
    x = int.from_bytes(bytes[1:49], 'big')
    y = int.from_bytes(bytes[49:97], 'big')
